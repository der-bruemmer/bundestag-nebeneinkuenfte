package de.ulei.nebeneinkuenfte.ui.solitaire;

import java.io.Serializable;
import java.util.ArrayList;
import java.util.List;

import org.vaadin.artur.playingcards.AbsoluteLayout;
...
public class Solitaire {
	...
	public Solitaire() {
		...
		mainLayout.setStyleName("Solitaire");
		createMainWindow(mainLayout);

		deck = new Deck();
		deck.addListener(new ClickListener() {

			private static final long serialVersionUID = 3524727365500867000L;
		...
		deckPile.addListener(new ClickListener() {
			...
			private static final long serialVersionUID = -4561417946340464370L;
		...
		for (int i = 0; i < stack.length; i++) {
			...
			stack[i].setDropHandler(new DropHandler() {
				...
				private static final long serialVersionUID = -8438810302115182971L;
			...
			stack[i].addListener(new LayoutClickListener() {
				...
				private static final long serialVersionUID = -467347820082708055L;
		...
		for (int i = 0; i < finalPile.length; i++) {
			...
			pile.addListener(new ClickListener() {
				...
				private static final long serialVersionUID = -3064963047686785623L;
			...
			pile.setDropHandler(new DropHandler() {
				 ...
				 * 
				 */
				private static final long serialVersionUID = 7068249001481244975L;
		...
		mainLayout.addComponent(deck, 0, 0);
		mainLayout.addComponent(deckPile, 0, Card.WIDTH + xOffset);
		for (int i = 0; i < finalPile.length; i++) {
			mainLayout.addComponent(finalPile[i], 0, (i + 3)
					* (Card.WIDTH + xOffset));
		}

		int stackY = Card.HEIGHT + yOffset;
		for (int i = 0; i < stack.length; i++) {
			mainLayout.addComponent(stack[i], stackY, i
					* (Card.WIDTH + xOffset));
		}
	...
	private class CardDropHandler implements DropHandler, Serializable {
		 ...
		 * 
		 */
		private static final long serialVersionUID = 1906856314504148521L;
		private Card card;
	...
	private void createMainWindow(AbsoluteLayout mainLayout) {
		createMenu();
		...
		CssLayout marginForMainLayout = new CssLayout();
		...
		mainWindow = new Window("Solitaire", wrapperLayout);
	...
	private void createMenu() {
		...
		settings.addItem("3 Karten", new ThemeResource(
			...
			public void menuSelected(MenuItem selectedItem) {
				if (selectedItem.getIcon() == null) {
					selectedItem.setIcon(new ThemeResource("menuchecked.png"));
		...
		gameMenu.addItem("Neu starten", new Command() {

			private static final long serialVersionUID = -8038076068225726532L;
	...
	public AbsoluteLayout getSolitaire() {
		return mainLayout;
	}

	public Window getSolitaireWindow() {
		return mainWindow;
	}
